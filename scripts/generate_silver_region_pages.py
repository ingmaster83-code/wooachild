#!/usr/bin/env python3
"""silverregion/{시도}/index.html 프론트매터 페이지 생성"""
import json
from pathlib import Path
from collections import Counter

ROOT = Path(__file__).parent.parent
DATA = json.loads((ROOT / "_rawdata" / "silverzone.json").read_text(encoding="utf-8"))
...
for region in sorted(counts):
    ...
    cnt = counts[region]
    d = ROOT / "silverregion" / region
    d.mkdir(parents=True, exist_ok=True)

    page_title = f"{full} 노인장애인보호구역"
    title_h1 = f"{full} 노인장애인보호구역 {cnt}곳"
    subtitle = f"{full} 지역 노인장애인보호구역 위치를 검색하세요."

    content = f"""---
layout: silverregion
title: {page_title}
description: {full} 노인장애인보호구역 정보. 경로당·양로원 등 노인·장애인 보호시설 주변 생활보호구역 위치와 CCTV 설치 현황을 확인하세요.
do_name: {region}
title_h1: {title_h1}
subtitle: {subtitle}
---
