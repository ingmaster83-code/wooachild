...
DATA = json.loads((ROOT / "_rawdata" / "zone.json").read_text(encoding="utf-8"))
...
for region in sorted(counts):
    ...
    page_title = f"{full} 어린이보호구역"
    title_h1 = f"{full} 어린이보호구역 {cnt}곳"
    subtitle = f"{full} 지역 어린이보호구역을 시설종류별로 검색하세요."

    content = f"""---
layout: region
title: {page_title}
description: {full} 어린이보호구역 정보. 초등학교·유치원·어린이집 주변 스쿨존 위치와 CCTV 설치 현황을 확인하세요.
do_name: {region}
title_h1: {title_h1}
subtitle: {subtitle}
---
